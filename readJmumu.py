...
tree = ROOT.TTree("nominal", "Truth info")
event  = array('f',[0])
...
for j in range(L):
        ...
        m1=[float(k) for k in lines[count+2].split(" ")]
        m2=[float(k) for k in lines[count+3].split(" ")]
        m3=[float(k) for k in lines[count+4].split(" ")]
        m4=[float(k) for k in lines[count+5].split(" ")]
        event[0]=e
        jpsi_1[0]=j1[0]
        #jpsi_2[0]=j2[0]
        jpsi_1[1]=j1[1]
